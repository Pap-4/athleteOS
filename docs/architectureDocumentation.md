...
- **Folders.** Layered groups files by technical role. All controllers together, all services together. Modular groups by feature, e.g everything about Nutrition lives in one folder.
...
One PostgreSQL database, one named Postgres schema per module. Rhe module boundary shows up in SQL itself, not only in application code:
...
Postgres enforces the schema boundary, but every module's tables are still defined in one shared `schema.prisma` file. That file is where the slices still overlap, even though the database itself keeps them apart.
